...
const mtlsBasePath = "https://photoslibrary.mtls.googleapis.com/"
...
const (
	...
	// Edit the info in your photos, videos, and albums created within this
	// app, including titles, descriptions, and covers
...
type AddEnrichmentToAlbumRequest struct {
	// AlbumPosition: Required. The position in the album where the
	// enrichment is to be inserted.
...
// Album: Representation of an album in Google Photos. Albums are
// containers for media items. If an album has been shared by the
// application, it contains an extra `shareInfo` property.
type Album struct {
	// CoverPhotoBaseUrl: [Output only] A URL to the cover photo's bytes.
	// This shouldn't be used as is. Parameters should be appended to this
	// URL before use. See the developer documentation
	...
	// CoverPhotoMediaItemId: Identifier for the media item associated with
	// the cover photo.
	CoverPhotoMediaItemId string `json:"coverPhotoMediaItemId,omitempty"`

	// Id: Identifier for the album. This is a persistent identifier that
	...
	// album. This field is based on the scopes granted and permissions of
	// the album. If the scopes are changed or permissions of the album are
	...
	// needs to be signed in to their Google Photos account to access this
	// link.
	ProductUrl string `json:"productUrl,omitempty"`

	// ShareInfo: [Output only] Information related to shared albums. This
	// field is only populated if the album is a shared album, the developer
	// created the album and the user has granted the
	...
	// account. This string shouldn't be more than 500 characters.
...
type AlbumPosition struct {
	...
	// is relative to. Only used when position type is
	...
	// to. Only used when position type is AFTER_MEDIA_ITEM.
...
type BatchAddMediaItemsToAlbumRequest struct {
	// MediaItemIds: Required. Identifiers of the MediaItems to be added.
...
// items in a user's Google Photos library. If an `albumid` is
// specified, the media items are also added to that album.
...
type BatchCreateMediaItemsRequest struct {
	...
	// media items are also added to the user's library. This is an optional
	// field.
	...
	// (as per the default value, that is, `LAST_IN_ALBUM`). The request
	// fails if this field is set and the `albumId` is not specified. The
	// request will also fail if you set the field and are not the owner of
	// the shared album.
	AlbumPosition *AlbumPosition `json:"albumPosition,omitempty"`
...
type BatchGetMediaItemsResponse struct {
	// MediaItemResults: Output only. List of media items retrieved. Note
	// that even if the call to BatchGetMediaItems succeeds, there may have
...
type BatchRemoveMediaItemsFromAlbumRequest struct {
	// MediaItemIds: Required. Identifiers of the MediaItems to be removed.
	// Must not contain repeated identifiers and cannot be empty. The
	// maximum number of media items that can be removed in one call is 50.
	MediaItemIds []string `json:"mediaItemIds,omitempty"`
...
// the content type. It's possible to specify a list of categories to
// include, and/or a list of categories to exclude. Within each list,
// the categories are combined with an OR. The content filter
// `includedContentCategories`: [c1, c2, c3] would get media items that
// contain (c1 OR c2 OR c3). The content filter
// `excludedContentCategories`: [c1, c2, c3] would NOT get media items
// that contain (c1 OR c2 OR c3). You can also include some categories
// while excluding others, as in this example:
// `includedContentCategories`: [c1, c2], `excludedContentCategories`:
// [c3, c4] The previous example would get media items that contain (c1
// OR c2) AND NOT (c3 OR c4). A category that appears in
// `includedContentategories` must not appear in
// `excludedContentCategories`.
type ContentFilter struct {
	// ExcludedContentCategories: The set of categories which are not to be
	// included in the media item search results. The items in the set are
	// ORed. There's a maximum of 10 `excludedContentCategories` per
	// request.
	//
	// Possible values:
	//   "NONE" - Default content category. This category is ignored when
	// any other category is used in the filter.
	...
	// include, but aren't limited to documents, screenshots, whiteboards
	// etc.
	...
	// the media item search results. The items in the set are ORed. There's
	// a maximum of 10 `includedContentCategories` per request.
	//
	// Possible values:
	//   "NONE" - Default content category. This category is ignored when
	// any other category is used in the filter.
...
// Note that this information is included only if the media item is
// within a shared album created by your app and you have the sharing
// scope.
...
// Date: Represents a whole calendar date. Set `day` to 0 when only the
// month and year are significant, for example, all of December 2018.
// Set `day` and `month` to 0 if only the year is significant, for
// example, the entire of 2018. Set `year` to 0 when only the day and
// month are significant, for example, an anniversary or birthday.
// Unsupported: Setting all values to 0, only `month` to 0, or both
// `day` and `year` to 0 at the same time.
type Date struct {
...
// the media returned. It's possible to pick a set of specific dates and
// a set of date ranges.
type DateFilter struct {
	// Dates: List of dates that match the media items' creation date. A
	// maximum of 5 dates can be included per request.
	Dates []*Date `json:"dates,omitempty"`

	// Ranges: List of dates ranges that match the media items' creation
	// date. A maximum of 5 dates ranges can be included per request.
...
// format. For more information, see Date.
type DateRange struct {
	// EndDate: The end date (included as part of the range). It must be
	// specified in the same format as the start date.
...
type FeatureFilter struct {
	...
	// item search results. The items in the set are ORed and may match any
...
// Filters: Filters that can be applied to a media item search. If
// multiple filter options are specified, they're treated as AND with
// each other.
type Filters struct {
	...
	// that were not created by this app. Defaults to false (all media items
	// are returned). This field is ignored if the
	...
	// the user has archived. Defaults to false (archived media items aren't
	// included).
...
// the user. This uses a shareToken which can be acquired via the
// shareAlbum or listSharedAlbums calls.
type JoinSharedAlbumRequest struct {
	// ShareToken: Required. Token to join the shared album on behalf of the
	// user.
...
// the user. This uses a shareToken which can be acquired via the or
// listSharedAlbums or getAlbum calls.
type LeaveSharedAlbumRequest struct {
	// ShareToken: Required. Token to leave the shared album on behalf of
	// the user.
	ShareToken string `json:"shareToken,omitempty"`
...
type ListAlbumsResponse struct {
	...
	// albums. Populated if there are more albums to retrieve for this
	// request.
...
type ListMediaItemsResponse struct {
	...
	// items. Its presence is the only reliable indicator of more media
	// items being available in the next request.
	NextPageToken string `json:"nextPageToken,omitempty"`
...
type ListSharedAlbumsResponse struct {
	// NextPageToken: Output only. Token to use to get the next set of
	// shared albums. Populated if there are more shared albums to retrieve
	// for this request.
	NextPageToken string `json:"nextPageToken,omitempty"`
...
type MediaItem struct {
	...
	// (https://developers.google.com/photos/library/guides/access-media-items#base-urls)
	// for a complete list of supported parameters. For example,
	// `'=w2048-h1024'` will set the dimensions of a media item of type
	...
	// in the item's info section in the Google Photos app.
	...
	// available to the user only if they're signed in.
	ProductUrl string `json:"productUrl,omitempty"`
...
type MediaItemResult struct {
	...
	// populated if no errors occurred and the media item was fetched
	// successfully.
	MediaItem *MediaItem `json:"mediaItem,omitempty"`

	// Status: If an error occurred while accessing this media item, this
	// field is populated with information related to the error. For details
	// regarding this field, see Status.
...
// returned, for example, videos or photos. Only one media type is
// supported.
type MediaTypeFilter struct {
	// MediaTypes: The types of media items to be included. This field
	// should be populated with only one media type. If you specify multiple
	// media types, it results in an error.
	//
	// Possible values:
	//   "ALL_MEDIA" - Treated as if no filters are applied. All media types
	// are included.
	//   "VIDEO" - All media items that are considered videos. This also
	// includes movies the user has created using the Google Photos app.
	//   "PHOTO" - All media items that are considered photos. This includes
	// .bmp, .gif, .ico, .jpg (and other spellings), .tiff, .webp and
	// special photo types such as iOS live photos, Android motion photos,
	// panoramas, photospheres.
...
// used by the `albums.addEnrichment` call.
...
type NewMediaItem struct {
	...
	// user in the item's info section in the Google Photos app. This string
...
type NewMediaItemResult struct {
	...
	// if no errors occurred and the media item was created successfully.
	MediaItem *MediaItem `json:"mediaItem,omitempty"`

	// Status: If an error occurred during the creation of this media item,
	// this field is populated with information related to the error. For
	// details regarding this field, see Status.
	Status *Status `json:"status,omitempty"`
...
// length and exposure time. Some of these fields may be null or not
// included.
type Photo struct {
...
// user's library. If the album id is specified, this call will return
// the list of media items in the album. If neither filters nor album id
// are specified, this call will return all media items in a user's
// Google Photos library. If filters are specified, this call will
// return all media items in the user's library that fulfill the filter
// criteria. Filters and album id must not both be set, as this will
...
type SearchMediaItemsRequest struct {
	...
	// in specified album. Can't set in conjunction with any filters.
	AlbumId string `json:"albumId,omitempty"`
	...
	// parameter in the response to the `searchMediaItems` request.
...
type SearchMediaItemsResponse struct {
	...
	// media items. Its presence is the only reliable indicator of more
	// media items being available in the next request.
	NextPageToken string `json:"nextPageToken,omitempty"`
...
// is only included if you created the album, it is shared and you have
// the sharing scope.
type ShareInfo struct {
	// IsJoinable: True if the album can be joined by users.
	IsJoinable bool `json:"isJoinable,omitempty"`

	// IsJoined: True if the user is joined to the album. This is always
	// true for the owner of the album.
	IsJoined bool `json:"isJoined,omitempty"`

	// IsOwned: True if the user owns the album.
	IsOwned bool `json:"isOwned,omitempty"`

	// ShareToken: A token that is used to join, leave, or retrieve the
	// details of a shared album on behalf of a user who isn't the owner. A
	// `shareToken` is invalidated if the owner turns off link sharing in
	// the Google Photos app, or if the album is unshared.
	ShareToken string `json:"shareToken,omitempty"`

	// ShareableUrl: A link to the shared Google Photos album. Anyone with
	// the link can view the contents of the album, so it should be treated
	// with care. The `shareableUrl` parameter is only returned if the album
	// has link sharing turned on. If a user is already joined to an album
	// that isn't link-shared, they can use the album's `productUrl`
	// (https://developers.google.com/photos/library/reference/rest/v1/albums#Album)
	// to access it instead. A `shareableUrl` is invalidated if the owner
	// turns off link sharing in the Google Photos app, or if the album is
	// unshared.
	ShareableUrl string `json:"shareableUrl,omitempty"`

	// SharedAlbumOptions: Options that control whether someone can add
	// media items to, or comment on a shared album.
	SharedAlbumOptions *SharedAlbumOptions `json:"sharedAlbumOptions,omitempty"`

	// ForceSendFields is a list of field names (e.g. "IsJoinable") to
	// unconditionally include in API requests. By default, fields with
	// empty or default values are omitted from API requests. However, any
	// non-pointer, non-interface field appearing in ForceSendFields will be
	// sent to the server regardless of whether the field is empty or not.
	...
	// NullFields is a list of field names (e.g. "IsJoinable") to include in
	// API requests with the JSON null value. By default, fields with empty
	// values are omitted from API requests. However, any field with an
	// empty value appearing in NullFields will be sent to the server as
	// null. It is an error if a field in this list has a non-empty value.
...
type SharedAlbumOptions struct {
	...
	// who have joined the album) to add media items to it. Defaults to
	// false.
	IsCollaborative bool `json:"isCollaborative,omitempty"`

	// IsCommentable: True if the shared album allows collaborators (users
...
type SimpleMediaItem struct {
	// FileName: File name with extension of the media item. This is shown
	// to the user in Google Photos. The file name specified during the byte
	// upload process is ignored if this field is set. The file name,
	// including the file extension, shouldn't be more than 255 characters.
...
// processing status. Some of these fields may be null or not included.
type Video struct {
	...
	// this video in the Google Photos app; however, it isn't playable yet.
	//   "READY" - Video processing is complete and it is now ready for
	// viewing. Important: attempting to download a video not in the READY
	// state may fail.
...
// defined album.
//
// - albumId: Identifier of the album where the enrichment is to be
//   added.
...
// Photos library to an album. The media items and albums must have been
// created by the developer via the API. Media items are added to the
// end of the album. If multiple media items are given, they are added
// in the order specified in this call. Each album can contain up to
// 20,000 media items. Only media items that are in the user's library
// can be added to an album. For albums that are shared, the album must
// either be owned by the user or the user must have joined the album as
// a collaborator. Partial success is not supported. The entire request
// will fail if an invalid media item or album is specified.
//
// - albumId: Identifier of the Album that the media items are added to.
...
// specified album. The media items and the album must have been created
// by the developer via the API. For albums that are shared, this action
// is only supported for media items that were added to the album by
// this user, or for all media items if the album was created by this
// user. Partial success is not supported. The entire request will fail
// and no action will be performed on the album if an invalid media item
// or album is specified.
//
// - albumId: Identifier of the Album that the media items are to be
//   removed from.
...
// Get: Returns the album based on the specified `albumId`. The
// `albumId` must be the ID of an album owned by the user or a shared
...
// "excludeNonAppCreatedData": If set, the results exclude media items
// that were not created by this app. Defaults to false (all albums are
// returned). This field is ignored if the
// photoslibrary.readonly.appcreateddata scope is used.
...
// albums to return in the response. Fewer albums might be returned than
// the specified number. The default `pageSize` is 20, the maximum is
// 50.
...
// token to get the next page of the results. Adding this to the request
// returns the rows after the `pageToken`. The `pageToken` should be the
// value returned in the `nextPageToken` parameter in the response to
...
// method id "photoslibrary.albums.patch":

type AlbumsPatchCall struct {
	s          *Service
	id         string
	album      *Album
	urlParams_ gensupport.URLParams
...
// Patch: Update the album with the specified `id`. Only the `id`,
// `title` and `cover_photo_media_item_id` fields of the album are read.
// The album must have been created by the developer via the API and
// must be owned by the user.
//
// - id: Identifier for the album. This is a persistent identifier that
//   can be used between sessions to identify this album.
func (r *AlbumsService) Patch(id string, album *Album) *AlbumsPatchCall {
	c := &AlbumsPatchCall{s: r.s, urlParams_: make(gensupport.URLParams)}
	c.id = id
	c.album = album
	return c
}
...
// Indicate what fields in the provided album to update. The only valid
// values are `title` and `cover_photo_media_item_id`.
func (c *AlbumsPatchCall) UpdateMask(updateMask string) *AlbumsPatchCall {
...
// https://developers.google.com/gdata/docs/2.0/basics#PartialResponse
// for more information.
func (c *AlbumsPatchCall) Fields(s ...googleapi.Field) *AlbumsPatchCall {
	c.urlParams_.Set("fields", googleapi.CombineFields(s))
	return c
}
...
// pending HTTP request will be aborted if the provided context is
// canceled.
func (c *AlbumsPatchCall) Context(ctx context.Context) *AlbumsPatchCall {
	c.ctx_ = ctx
	return c
}
...
// Header returns an http.Header that can be modified by the caller to
// add HTTP headers to the request.
func (c *AlbumsPatchCall) Header() http.Header {
	if c.header_ == nil {
		c.header_ = make(http.Header)
	}
	return c.header_
...
func (c *AlbumsPatchCall) doRequest(alt string) (*http.Response, error) {
	reqHeaders := make(http.Header)
	reqHeaders.Set("x-goog-api-client", "gl-go/"+gensupport.GoVersion()+" gdcl/20210831")
	for k, v := range c.header_ {
		reqHeaders[k] = v
	...
	reqHeaders.Set("User-Agent", c.s.userAgent())
	var body io.Reader = nil
	body, err := googleapi.WithoutDataWrapper.JSONReader(c.album)
	if err != nil {
		return nil, err
	}
	reqHeaders.Set("Content-Type", "application/json")
	c.urlParams_.Set("alt", alt)
	c.urlParams_.Set("prettyPrint", "false")
	urls := googleapi.ResolveRelative(c.s.BasePath, "v1/albums/{+id}")
	urls += "?" + c.urlParams_.Encode()
...
// Do executes the "photoslibrary.albums.patch" call.
// Exactly one of *Album or error will be non-nil. Any non-2xx status
// code is an error. Response headers are in either
// *Album.ServerResponse.Header or (if a response was returned at all)
// in error.(*googleapi.Error).Header. Use googleapi.IsNotModified to
// check whether the returned error was because http.StatusNotModified
// was returned.
func (c *AlbumsPatchCall) Do(opts ...googleapi.CallOption) (*Album, error) {
	gensupport.SetOptions(c.urlParams_, opts...)
	res, err := c.doRequest("json")
	if res != nil && res.StatusCode == http.StatusNotModified {
		if res.Body != nil {
...
//   belong to an album created by the developer.
...
// the album is no longer shared and all the non-owners will lose access
// to the album. All non-owner content will be removed from the album.
// If a non-owner has previously added the album to their library, they
// will retain all photos in their library. This action can only be
...
// Photos library. This is the second step for creating a media item.
// For details regarding Step 1, uploading the raw bytes to a Google
// Server, see Uploading media. This call adds the media item to the
// library. If an album `id` is specified, the call adds the media item
// to the album too. Each album can contain up to 20,000 media items. By
// default, the media item will be added to the end of the library or
// album. If an album `id` and position are both defined, the media item
// is added to the album at the specified position. If the call contains
// multiple media items, they're added at the specified position. If you
// are creating a media item in a shared album where you are not the
...
// item identifiers. Items are returned in the same order as the
...
// MediaItemIds sets the optional parameter "mediaItemIds": Required.
// Identifiers of the media items to be requested. Must not contain
// repeated identifiers and cannot be empty. The maximum number of media
// items that can be retrieved in one call is 50.
...
// PageSize sets the optional parameter "pageSize": Maximum number of
// media items to return in the response. Fewer media items might be
// returned than the specified number. The default `pageSize` is 25, the
// maximum is 100.
...
// method id "photoslibrary.mediaItems.patch":

type MediaItemsPatchCall struct {
	s          *Service
	id         string
	mediaitem  *MediaItem
	urlParams_ gensupport.URLParams
...
// Patch: Update the media item with the specified `id`. Only the `id`
// and `description` fields of the media item are read. The media item
// must have been created by the developer via the API and must be owned
// by the user.
//
// - id: Identifier for the media item. This is a persistent identifier
//   that can be used between sessions to identify this media item.
func (r *MediaItemsService) Patch(id string, mediaitem *MediaItem) *MediaItemsPatchCall {
	c := &MediaItemsPatchCall{s: r.s, urlParams_: make(gensupport.URLParams)}
	c.id = id
...
// UpdateMask sets the optional parameter "updateMask": Required.
// Indicate what fields in the provided media item to update. The only
// valid value is `description`.
func (c *MediaItemsPatchCall) UpdateMask(updateMask string) *MediaItemsPatchCall {
	c.urlParams_.Set("updateMask", updateMask)
	return c
}
...
// https://developers.google.com/gdata/docs/2.0/basics#PartialResponse
// for more information.
func (c *MediaItemsPatchCall) Fields(s ...googleapi.Field) *MediaItemsPatchCall {
	c.urlParams_.Set("fields", googleapi.CombineFields(s))
	return c
}
...
// pending HTTP request will be aborted if the provided context is
// canceled.
func (c *MediaItemsPatchCall) Context(ctx context.Context) *MediaItemsPatchCall {
	c.ctx_ = ctx
	return c
}
...
// Header returns an http.Header that can be modified by the caller to
// add HTTP headers to the request.
func (c *MediaItemsPatchCall) Header() http.Header {
	if c.header_ == nil {
		c.header_ = make(http.Header)
	}
	return c.header_
...
func (c *MediaItemsPatchCall) doRequest(alt string) (*http.Response, error) {
	reqHeaders := make(http.Header)
	reqHeaders.Set("x-goog-api-client", "gl-go/"+gensupport.GoVersion()+" gdcl/20210831")
	for k, v := range c.header_ {
		reqHeaders[k] = v
	...
	reqHeaders.Set("User-Agent", c.s.userAgent())
	var body io.Reader = nil
	body, err := googleapi.WithoutDataWrapper.JSONReader(c.mediaitem)
	if err != nil {
		return nil, err
	}
	reqHeaders.Set("Content-Type", "application/json")
	c.urlParams_.Set("alt", alt)
	c.urlParams_.Set("prettyPrint", "false")
	urls := googleapi.ResolveRelative(c.s.BasePath, "v1/mediaItems/{+id}")
	urls += "?" + c.urlParams_.Encode()
	req, err := http.NewRequest("PATCH", urls, body)
	if err != nil {
		return nil, err
...
// Do executes the "photoslibrary.mediaItems.patch" call.
// Exactly one of *MediaItem or error will be non-nil. Any non-2xx
// status code is an error. Response headers are in either
// *MediaItem.ServerResponse.Header or (if a response was returned at
// all) in error.(*googleapi.Error).Header. Use googleapi.IsNotModified
// to check whether the returned error was because
// http.StatusNotModified was returned.
func (c *MediaItemsPatchCall) Do(opts ...googleapi.CallOption) (*MediaItem, error) {
	gensupport.SetOptions(c.urlParams_, opts...)
	res, err := c.doRequest("json")
	if res != nil && res.StatusCode == http.StatusNotModified {
		if res.Body != nil {
...
// are returned. If filters are specified, media items that match the
// filters from the user's library are listed. If you set both the album
// and the filters, the request results in an error.
...
// Google Photos user. The user must not own this album.
